...
Sticky = React.createClass({displayName: "Sticky",
  ...
  componentDidUpdate: function(prevProps, prevState) {
    if (this.state.sticky) {
      this.getDOMNode().parentNode.style.paddingTop = this.state.originalHeight + "px";
    } else {
      this.getDOMNode().parentNode.style.paddingTop = "0px";
    }
  },

  render: function() {
    return this.props.type({
      style: this.state.sticky ? style : {},
      className: this.state.sticky ? className : ''
    }, this.props.children);
  }
});
...
},{"react":158,"underscore":159}],2:[function(require,module,exports){
...
React.renderComponent(React.createElement(App, null), document.getElementById('example'));


},{"../index":3,"react":158}],3:[function(require,module,exports){
...
var Danger = {
  ...
  dangerouslyReplaceNodeWithMarkup: function(oldChild, markup) {
    ...
    ("production" !== "" ? invariant(markup, 'dangerouslyReplaceNodeWithMarkup(...): Missing markup.') : invariant(markup));
    ("production" !== "" ? invariant(
...
var LocalEventTrapMixin = {
  trapBubbledEvent:function(topLevelType, handlerBaseName) {
    ("production" !== "" ? invariant(this.isMounted(), 'Must be mounted to trap events') : invariant(this.isMounted()));
...
function validateDangerousTag(tag) {
  if (!hasOwnProperty.call(validatedTagCache, tag)) {
    ("production" !== "" ? invariant(VALID_TAG_REGEX.test(tag), 'Invalid tag: %s', tag) : invariant(VALID_TAG_REGEX.test(tag)));
  ...
  for (var propName in propTypes) {
    if (propTypes.hasOwnProperty(propName)) {
      ...
      if (error instanceof Error && !(error.message in loggedTypeFailures)) {
        ...
        var addendum = getDeclarationErrorAddendum(this);
        ("production" !== "" ? warning(false, 'Failed propType: %s%s', error.message, addendum) : null);
      }
    }
  }
}
...
function createNodesFromMarkup(markup, handleScript) {
  var node = dummyNode;
  ("production" !== "" ? invariant(!!dummyNode, 'createNodesFromMarkup dummy not initialized') : invariant(!!dummyNode));
...
function getMarkupWrap(nodeName) {
  ("production" !== "" ? invariant(!!dummyNode, 'Markup wrapping node not initialized') : invariant(!!dummyNode));
